...
class CitationStorer(object):
    ...
    def __init__(self, dir_data_path, rdf_resource_base,
        ...
        self.rdf_dir_local_path = "rdf" + sep + self.cur_time[:7].replace("-", sep) + sep
        self.slx_dir_local_path = "slx" + sep + self.cur_time[:7].replace("-", sep) + sep
    ...
    @staticmethod
    def __get_right_file_path(base_dir, partial_file_name, file_ext, threshold, cit_counter=None):
        final_index = 1

        for f_path in glob(base_dir + sep + partial_file_name + "_*." + file_ext):
            cur_index = int(sub("^.+_([0-9]+)\.%s$" % file_ext, "\\1", basename(f_path)))
        ...
        if cit_counter is None or number_of_citations >= threshold:
            number_of_citations = 0
            final_file_path = sub("_[0-9]+\.%s$" % file_ext, "_%s.%s" % (
                str(final_index + 1), file_ext), final_file_path)
    ...
    def load_citations_from_file(data_f_path, prov_f_path=None, oci="04201-04201", baseurl="",
        ...
        if exists(data_f_path) and isfile(data_f_path):
            if prov_f_path is not None and exists(prov_f_path) and isfile(prov_f_path):
                if data_f_path.endswith("." + CitationStorer.CSV_EXT) and \
                        prov_f_path.endswith("." + CitationStorer.CSV_EXT):
                    result.extend(CitationStorer.__load_citations_from_csv_file(
                        data_f_path, prov_f_path, baseurl, service_name, id_type, id_shape, citation_type))
                elif data_f_path.endswith("." + CitationStorer.RDF_EXT) and \
                        prov_f_path.endswith("." + CitationStorer.RDF_EXT):
                    result.extend(CitationStorer.__load_citations_from_rdf_file(
                        data_f_path, prov_f_path, service_name, id_type, id_shape, citation_type))
            elif data_f_path.endswith("." + CitationStorer.SLX_EXT):
                result.extend(CitationStorer.__load_citations_from_slx_file(
    ...
    def __load_citations_from_csv_file(data_f_path, prov_f_path, baseurl, service_name,
        ...
        for oci in citation_data:
            ...
            c = Citation(oci, baseurl + quote(dent["citing"]), None, baseurl + quote(dent["cited"]), None,
                         dent["creation"], dent["timespan"],
                         int(pent["snapshot"]), pent["agent"], pent["source"], pent["created"],
                         service_name, id_type, id_shape, citation_type,
                         dent["journal_sc"] == "yes", dent["author_sc"] == "yes",
                         pent["invalidated"], pent["description"], pent["update"])
    ...
    def __load_citations_from_rdf_file(data_f_path, prov_f_path, service_name,
        ...
        for cit_ent in citation_data.subjects(RDF.type, Citation.citation):
            ...
            for en in citation_prov.objects(prov_entity, Citation.has_update_query):
                update = str(en)
            for en in citation_data.objects(cit_ent, Citation.has_citation_creation_date):
            ...
            c = Citation(sub("^.+/ci/(.+)$", "\\1", str(cit_ent)),
                         ...
                         creation_date, timespan, entity_snapshot,
                         str(list(citation_prov.objects(prov_entity, Citation.was_attributed_to))[0]),
                         str(list(citation_prov.objects(prov_entity, Citation.had_primary_source))[0]),
                         str(list(citation_prov.objects(prov_entity, Citation.generated_at_time))[0]),
                         service_name, id_type, id_shape, citation_type,
                         Citation.journal_self_citation in citation_data.objects(cit_ent, RDF.type),
                         Citation.author_self_citation in citation_data.objects(cit_ent, RDF.type),
                         invalidated,
                         str(list(citation_prov.objects(prov_entity, Citation.description))[0]), update)
    ...
    def __load_citations_from_slx_file(data_f_path, oci, service_name, id_type, id_shape,
        ...
        with open(data_f_path, encoding="utf8") as f:
            ...
            for obj in citation_data:
                c = Citation(oci,
                             obj["Source"]["Identifier"]["IDURL"], obj["Source"].get("PublicationDate"),
                             obj["Target"]["Identifier"]["IDURL"], obj["Target"].get("PublicationDate"),
                             None, None, 1, agent, source, obj["LinkPublicationDate"],
                             service_name, id_type, id_shape, citation_type, False, False, None, None, None)
    ...
    def store_citation(self, citation):
        ...
        CitationStorer.__store_csv_on_file(prov_csv_f_path, Citation.header_provenance_data,
        ...
        CitationStorer.__store_rdf_on_file(data_rdf_f_path,
                                           citation.get_citation_rdf(self.rdf_resource_base,
                                                                     False, False, False))
        CitationStorer.__store_rdf_on_file(prov_rdf_f_path,
                                           citation.get_citation_prov_rdf(self.rdf_resource_base), "nq")

        # Store data in Scholix
        slx_filename = self.get_slx_filename(True)
        data_slx_f_path = self.data_slx_dir + slx_filename

        CitationStorer.__store_slx_on_file(data_slx_f_path, citation.get_citation_scholix())
