...
class CitationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.citation_1 = Citation(
            ...
            "http://dx.doi.org/10.1038/sj.cdd.4401289", "2003-10-24",
            "http://dx.doi.org/10.1096/fj.00-0336fje", "2001-01",
            None, None,
            1, "https://w3id.org/oc/index/prov/ra/1",
            "https://api.crossref.org/works/10.1038/sj.cdd.4401289", "2018-11-01T09:14:03",
        ...
        self.citation_2 = Citation(
            ...
            "http://dx.doi.org/10.1002/jrs.5400", "2018-06",
            "http://dx.doi.org/10.1002/jrs.5302", "2017-12-05",
            None, None,
            1, "https://w3id.org/oc/index/prov/ra/1",
            "https://api.crossref.org/works/10.1002/jrs.5400", "2018-11-01T14:51:52",
            "OpenCitations Index: COCI", "doi", "http://dx.doi.org/([[XXX__decode]])", None,
            journal_sc=True, author_sc=True, prov_description="Creation of the citation")

        self.citation_3 = Citation(
            "02001000002361927283705040000-020010003093612062710020603000720",
            "http://dx.doi.org/10.1002/jrs.5400", "2018-06",
            "http://dx.doi.org/10.1039/c6ra26307k", "2017",
            None, None,
            1, "https://w3id.org/oc/index/prov/ra/1",
            "https://api.crossref.org/works/10.1002/jrs.5400", "2018-11-01T14:51:52",
            "OpenCitations Index: COCI", "doi", "http://dx.doi.org/([[XXX__decode]])", None,
            journal_sc=False, author_sc=True, prov_description="Creation of the citation")

        self.citation_4 = Citation(
            "02001000308362819371213133704040001020804-02001000308362819371213133704040000030707",
            "http://dx.doi.org/10.1038/sj.cdd.4401284", "2003-08-22",
            "http://dx.doi.org/10.1038/sj.cdd.4400377", "1998-05",
            None, None,
            1, "https://w3id.org/oc/index/prov/ra/1",
            "https://api.crossref.org/works/10.1038/sj.cdd.4401284", "2018-11-01T09:14:03",
            "OpenCitations Index: COCI", "doi", "http://dx.doi.org/([[XXX__decode]])", None,
            journal_sc=True, author_sc=False, prov_description="Creation of the citation")

        self.citation_5 = Citation(
            "020010000023625242110370100030001-02001010009361222251430273701090809370903040403",
            "http://dx.doi.org/10.1002/pola.10301", "2002-06-21",
            "http://dx.doi.org/10.1109/cmpeur.1989.93443", None,
            None, None,
            1, "https://w3id.org/oc/index/prov/ra/1",
            "https://api.crossref.org/works/10.1002/pola.10301", "2018-10-31T16:13:26",
            "OpenCitations Index: COCI", "doi", "http://dx.doi.org/([[XXX__decode]])", None,
            journal_sc=False, author_sc=False, prov_description="Creation of the citation")
    ...
    def test_citation_data_csv(self):
        ...
        self.assertEqual(list(DictReader(StringIO(self.citation_2.get_citation_csv())))[0], citation_data_csv[1])
        self.assertEqual(list(DictReader(StringIO(self.citation_3.get_citation_csv())))[0], citation_data_csv[2])
        self.assertEqual(list(DictReader(StringIO(self.citation_4.get_citation_csv())))[0], citation_data_csv[3])
        self.assertEqual(list(DictReader(StringIO(self.citation_5.get_citation_csv())))[0], citation_data_csv[4])
        self.assertEqual(list(DictReader(StringIO(self.citation_6.get_citation_csv())))[0], citation_data_csv[5])
    ...
    def test_citation_prov_csv(self):
        ...
        self.assertEqual(list(DictReader(StringIO(self.citation_1.get_citation_prov_csv())))[0],
                         citation_prov_csv[0])
        self.assertEqual(list(DictReader(StringIO(self.citation_2.get_citation_prov_csv())))[0],
                         citation_prov_csv[1])
        self.assertEqual(list(DictReader(StringIO(self.citation_3.get_citation_prov_csv())))[0],
                         citation_prov_csv[2])
        self.assertEqual(list(DictReader(StringIO(self.citation_4.get_citation_prov_csv())))[0],
                         citation_prov_csv[3])
        self.assertEqual(list(DictReader(StringIO(self.citation_5.get_citation_prov_csv())))[0],
                         citation_prov_csv[4])
        self.assertEqual(list(DictReader(StringIO(self.citation_6.get_citation_prov_csv())))[0],
                         citation_prov_csv[5])
    ...
    def test_citation_data_ttl(self):
        ...
        g2 = ConjunctiveGraph()
        for c in [self.citation_1, self.citation_2, self.citation_3,
    ...
    def test_citation_prov_ttl(self):
        ...
        for c in [self.citation_1, self.citation_2, self.citation_3,
                  self.citation_4, self.citation_5, self.citation_6]:
            for s, p, o, g in c.get_citation_prov_rdf(self.base_url).quads((None, None, None, None)):
                g2.add((s, p, o, g))
    ...
    def test_citation_data_prov_scholix(self):
        ...
        self.assertEqual(loads(self.citation_2.get_citation_scholix()), citation_data_prov_scholix[1])
        self.assertEqual(loads(self.citation_3.get_citation_scholix()), citation_data_prov_scholix[2])
        self.assertEqual(loads(self.citation_4.get_citation_scholix()), citation_data_prov_scholix[3])
        self.assertEqual(loads(self.citation_5.get_citation_scholix()), citation_data_prov_scholix[4])
        self.assertEqual(loads(self.citation_6.get_citation_scholix()), citation_data_prov_scholix[5])
    ...
    def test_lookup(self):
        ...
        self.assertEqual(doi_1.replace("10.", "", 1),
                         "".join([oci_man.lookup[code] for code in findall("(9*[0-8][0-9])",
                                                                          oci.replace("oci:", "").split(
                                                                              "-")[0].replace("020", "", 1))]))
        self.assertEqual(doi_2.replace("10.", "", 1),
                         "".join([oci_man.lookup[code] for code in findall("(9*[0-8][0-9])",
                                                                          oci.replace("oci:", "").split(
                                                                              "-")[1].replace("020", "", 1))]))
        ...
        self.assertEqual(doi_3.replace("10.", "", 1),
                         "".join([oci_man.lookup[code] for code in findall("(9*[0-8][0-9])",
                                                                           oci.replace("oci:", "").split(
                                                                               "-")[0].replace("020", "", 1))]))
        self.assertEqual(doi_4.replace("10.", "", 1),
                         "".join([oci_man.lookup[code] for code in findall("(9*[0-8][0-9])",
                                                                           oci.replace("oci:", "").split(
                                                                               "-")[1].replace("020", "", 1))]))
        self.assertEqual(len(oci_man.lookup.keys()), len(set(doi_1 + doi_2 + doi_3 + doi_4)))

        # Test conversion with incomplete file (ver 2, non-existing DOIs)
        oci_man = OCIManager(lookup_file=new_file_path)
        oci = oci_man.get_oci(doi_5, doi_6, "020")
        self.assertEqual(doi_5.replace("10.", "", 1),
                         "".join([oci_man.lookup[code] for code in findall("(9*[0-8][0-9])",
                                                                           oci.replace("oci:", "").split(
                                                                               "-")[0].replace("020", "", 1))]))
        self.assertEqual(doi_6.replace("10.", "", 1),
                         "".join([oci_man.lookup[code] for code in findall("(9*[0-8][0-9])",
                                                                           oci.replace("oci:", "").split(
                                                                               "-")[1].replace("020", "", 1))]))
        self.assertEqual(len(oci_man.lookup.keys()), len(set(doi_1 + doi_2 + doi_3 + doi_4 + doi_5 + doi_6)))
