...
class MetaIDManager(IdentifierManager):
    ...
    def is_valid(
        self, id_string
    ):  #  verifies if is valid and is in the list of valid metaids
    ...
    def normalise(self, id_string, include_prefix=False):
        try:
            metaid_string = sub(
                "\0+",
                "",
                sub("\s+", "", unquote(id_string[id_string.index("/") - 2 :])),
            )
            return "%s%s" % (
                self.p if include_prefix else "",
                metaid_string.lower().strip(),
            )
    ...
    def __metaid_exists(self, metaid_full):
        if self._use_api_service:
            ...
            while tentative:
                ...
                try:
                    r = get(
                        self._api + quote(metaid), headers=self._headers, timeout=30
                    )
