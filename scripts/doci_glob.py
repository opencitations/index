                ...
                if id_date.get_value(citing_doi) is None:
                    ...
                    if listDates != []:
                        if [data for data in listDates if str(data['dateType']).lower() == 'issued']:
                            ...
                            if [iss_date for iss_date in issue_dates if valid_date_doci(str(iss_date['date']))]:
                                flt_issue_dates = [iss_date for iss_date in issue_dates if
                                                   valid_date_doci(str(iss_date['date']))]
                                for flt_iss_date in flt_issue_dates:
                                    id_date.add_value(citing_doi, valid_date_doci(str(flt_iss_date['date'])))
                        ...
                        if container['identifier'] != "" and (container['identifierType']).lower() == "issn":
                            ...
                            if 'title' in container.keys():
                                ...
                                if journal_title in issnDict.keys():
                                    ...
                                    if issnList != []:
                                        if [el for el in issnList if el not in issn_set]:
                            ...
                            if relationType.lower() in relevant_relations:
                                if 'relatedIdentifierType' in related.keys():
                                    ...
                                    if relatedIdentifierType == 'doi':
                                        if 'relatedIdentifier' in related.keys():
                                            relatedDOI = doi_manager.normalise(related['relatedIdentifier'], True)
...
def main():
    ...
    process_doci(args.input_dir, args.output_dir, args.num_entities)
