        ...
        if "data" in data:
            ...
            for item in tqdm(data_list):
                ...
                if id_date.get_value(citing_doi) is None:
                    ...
                    if listDates != []:
                        if [data for data in listDates if str(data['dateType']).lower() == 'issued']:
                            issue_dates = [data for data in listDates if str(data['dateType']).lower() == 'issued']
                            if [iss_date for iss_date in issue_dates if valiDate(str(iss_date['date']))]:
                                flt_issue_dates = [iss_date for iss_date in issue_dates if valiDate(str(iss_date['date']))]
                                for flt_iss_date in flt_issue_dates:
                                    id_date.add_value(citing_doi, valiDate(str(flt_iss_date['date'])))
                ...
                if int(count) != 0 and int(count) % int(n) == 0:
                    print("trasferimento cash dati issn. Entità processate:", count, ". Trascrizione cache n.", (int(count)//int(n)))
                            ...
                            if relationType.lower() in relevant_relations:
                                if 'relatedIdentifierType' in related.keys():
                                    ...
                                    if relatedIdentifierType == 'doi':
                                        if 'relatedIdentifier' in related.keys():
                                            relatedDOI = doi_manager.normalise(str(related['relatedIdentifier']))
                                            if valid_doi.get_value(relatedDOI) is None or valid_doi.get_value(relatedDOI) == "i":
                                                # condizione aggiunta per evitare di validare di nuovo tramite API doi già validati
                                                valid_doi.add_value(relatedDOI, "v" if doi_manager.is_valid(relatedDOI) else "i")
                                                cited_dois += 1
...
def main():
    ...
    process(args.input_dir, args.output_dir, args.num_entities)

# Added for testing purposes, in the official version it should be removed
if __name__ == '__main__':
    main()

# GitHub\index>python "scripts/doci_glob.py" -i ./index/python/test/data/doci_glob_dump_input -o ./index/python/test/data/doci_glob_dump_output -n 50
